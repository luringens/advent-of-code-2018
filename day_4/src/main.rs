...
fn main() {
    let data = parse_data();
    let most_asleep = data
        .iter()
        .max_by_key(|(_, v)| {
            v.iter()
                .map(|(d1, d2)| d2.signed_duration_since(*d1).num_minutes())
                .sum::<i64>()
        })
        .unwrap();
        ...
        match line.split_whitespace().last().unwrap() {
            ...
            "up" => {
                if map.contains_key(&current_guard_id) {
                    map.get_mut(&current_guard_id)
                        .unwrap()
                        .push((current_guard_asleep.clone(), date));
                } else {
